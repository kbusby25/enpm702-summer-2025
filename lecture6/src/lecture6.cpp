...
int main() {
  ...
  //     std::cout << *u << '\n';  // 20
  //     std::cout << u << '\n';   // Compilation error: no operator<< for
  //     unique_ptr
  ...
  //     weak_observer = shared_resource;  // weak_observer now observes the
  //     resource if (auto locked_ptr = weak_observer.lock(); locked_ptr)
  ...
  //     auto shared_resource = std::make_shared<int>(10);
  //     weak_observer = shared_resource;  // weak_observer now observes the
  //     resource if (!weak_observer.expired())
