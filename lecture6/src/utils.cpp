...
void sink_shared_ptr(
    std::shared_ptr<int> ptr) {         // Copy constructor increments ref count
  std::cout << ptr.use_count() << '\n'; // 2 (original + copy)
}

void reseat_shared_ptr(
    std::shared_ptr<int> &ptr) {        // Pass by reference - no copy
